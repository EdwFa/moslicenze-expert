...
class AnalyticalOrchestrator:
    """
    Agent 4: Central Analytical Engine & Orchestrator.
    Uses LangGraph to coordinate checking logic.
    """
    def __init__(self):
    ...
    def classify_and_parse_node(self, state: ExpertiseState) -> Dict:
        """
        Runs Agent 1 and Agent 2 for all provided documents.
        """
        results = []
        all_extracted = {}
        findings = []
    ...
    def cross_document_check_node(self, state: ExpertiseState) -> Dict:
        """
        Performs logic checks (INN match, fee amount, etc.)
        """
        ...
        # Logic 3: Property Cadastral Number Check
    ...
    def mcp_validation_node(self, state: ExpertiseState) -> Dict:
        """
        Calls Agent 6 (MCP) to validate address and KPP.
        """
        import asyncio
        ...
        try:
            # Running async MCP client in a sync node
            import json
    ...
    def finalize_expertise_node(self, state: ExpertiseState) -> Dict:
        """
        Determines overall status and draft decision.
        """
        findings = state["analysis_findings"]
    ...
    def generate_report_node(self, state: ExpertiseState) -> Dict:
        """
        Calls Agent 5 to generate the final report.
        """
